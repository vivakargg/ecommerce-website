...
function useMounted() {
  ...
  const handleGenerate = () => {
    ...
    generate({
      ...
      productFamily: styleParam.split("-").map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(" "),
          ...
          {isGenerating && (
            ...
            >
              ...
              <div className="text-center px-5">
                <h2 className="text-2xl font-bold text-white mb-2 tracking-tight">Generating Studio Pack</h2>
                <p className="text-[#99A1AF] text-sm animate-pulse font-medium">Rendering {selectedViews.length} editorial angles for {product}...</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
