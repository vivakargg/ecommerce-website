...
export default function OutputResultsPage() {
  ...
  useEffect(() => {
    if (isMounted && !currentProject?.outputViews?.length && !currentProject?.primeImage) {
      router.replace(`/apparel/${segment}/${styleParam}/views`);
    }
  ...
  const results: ResultItem[] = [
    ...
    ...(currentProject?.videoUrl
      ? [{ title: "Straight Walk", image: currentProject?.primeImage || "", videoUrl: currentProject.videoUrl, isVideo: true }]
      : []),
  ];
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      saveAs(content, `${segment}_${styleParam.replace(/\s+/g, "_")}_views.zip`);
    } catch (err) {
