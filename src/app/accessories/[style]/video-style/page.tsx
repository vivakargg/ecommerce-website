...
export default function AccessoriesVideoStyleSelectionPage() {
  ...
  const videoStyles = [
    ...
    { id: "slow-pan", title: "Cinematic Pan" },
    { id: "dynamic-zoom", title: "Detail Zoom" },
    { id: "lifestyle-motion", title: "Lifestyle Aesthetic" }
  ];
  ...
  useEffect(() => {
    if (isCompleted && outputVideo) {
      ...
      router.push(`/accessories/${styleParam}/video/result?product=${product}`);
    }
        ...
        {(!currentProject?.sourceJobId && !isLoading) && (
          ...
          >
            ...
            <p className="text-amber-400/80 text-xs font-medium ml-8">
              Please 
              <button 
                onClick={() => router.push(`/accessories/${styleParam}/approve-prime?product=${product}`)}
