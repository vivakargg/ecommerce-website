...
export default function AccessoriesOutputResultsPage() {
  ...
  useEffect(() => {
    if (isMounted && !currentProject?.outputViews?.length && !currentProject?.primeImage) {
      router.replace(`/accessories/${styleParam}/views?product=${product}`);
    }
  ...
  const results: ResultItem[] = [
    ...
    ...(currentProject?.videoUrl
      ? [{ title: "Motion Synthesis", image: currentProject?.primeImage || "", videoUrl: currentProject.videoUrl, isVideo: true }]
      : []),
  ];
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      saveAs(content, `Accessories_${styleParam}_views.zip`);
    } catch (err) {
