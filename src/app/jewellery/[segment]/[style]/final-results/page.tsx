...
import { Download, Play, RefreshCcw, Camera, PlayCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function JewelleryResultPage() {
  ...
  const results: ResultItem[] = [
    ...
    ...(currentProject?.videoUrl ? [{ title: "Motion Shine", image: currentProject?.primeImage || "", videoUrl: currentProject.videoUrl, isVideo: true }] : []),
  ];
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      saveAs(content, `Jewellery_${segment}_${styleParam.replace(/\s+/g, "_")}_results.zip`);
    } catch (error) {
    ...
    <div className="relative min-h-screen bg-black text-white selection:bg-figma-gradient/30 font-roboto">
      ...
      <main className="w-full max-w-full lg:max-w-7xl mx-auto pt-[120px] px-5 pb-20 flex flex-col items-center">
        ...
        <div className="w-full max-w-[353px] flex gap-3 mb-6">
          <button 
            onClick={() => router.push(`/jewellery/${segment}/${styleParam}/views`)}
