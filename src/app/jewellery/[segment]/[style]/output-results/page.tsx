...
export default function JewelleryOutputResultsPage() {
  ...
  useEffect(() => {
    if (isMounted && !currentProject?.outputViews?.length) {
      router.replace(`/jewellery/${segment}/${styleParam}/views`);
    }
  }, [isMounted, currentProject, router, segment, styleParam]);
  ...
  const results: ResultItem[] = [
    ...
    ...(currentProject?.videoUrl
      ? [{ title: "Motion Shine", image: currentProject?.primeImage || "", videoUrl: currentProject.videoUrl, isVideo: true }]
      : []),
  ];
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      saveAs(content, `Jewellery_${segment}_${styleParam}_views.zip`);
    } catch (err) {
    ...
    <div className="relative min-h-screen bg-black text-white font-roboto">
      ...
      <main className="w-full max-w-full lg:max-w-7xl mx-auto pt-[120px] px-5 pb-20 flex flex-col items-center">
        ...
        <div className="w-full max-w-[353px] mb-4">
          <button
            onClick={() => router.push(`/jewellery/${segment}/${styleParam}/video-style`)}
